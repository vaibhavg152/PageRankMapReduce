...
using namespace std;
typedef std::pair< std::vector<int>, std::vector<float> > keyvalue_t;
...
void function_for_map(keyvalue_t &key_values, int index){
    ...
    if(size != 0){
        for(int i(0);i<size;i++){
            ...
            (key_values.first).push_back(graph.at(index).at(i));
            (key_values.second).push_back(passedImportance);
        }
    }
    if(node_exists[index]){
        ...
        (key_values.first).push_back(index);
        (key_values.second).push_back(dImportance);
    }
}
...
public:
    ...
    void COLLATE(){
        ...
        if(my_rank!=root){
            ...
            int kv_size = (kv.first).size();
            MPI_Send(&kv_size, 1, MPI_INT, root, 0, communicator);
            MPI_Send(&kv.first[0], kv_size, MPI_INT, root, 0, communicator);
            MPI_Send(&kv.second[0], kv_size, MPI_FLOAT, root, 0, communicator);
            ...
            for(int i=0; i<(kv.first).size(); i++){
                ...
                if(it==all_keymultivalues.end()){
                    std::vector<float> v = {kv.second[i]};
                    all_keymultivalues.insert({key,v});
                }
                else{
                    std::vector<float> v = it->second;
                    v.push_back(kv.second[i]);
    ...
    void print_keyvalue(){
        ...
        for(int i=0; i<kv.first.size(); i++){
            std::cout << "key:"<<kv.first[i]<<" value:"<<kv.second[i] << '\n';
        }
    }
